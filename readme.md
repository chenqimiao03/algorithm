...
1. 当前位置的**左侧**比当前位置的数字**小/大，且距离最近的位置**在哪。
2. 当前位置的**右侧**比当前位置的数字**小/大，且距离最近的位置**在哪。
...
查询某个元素时，该元素经过 `k` 个哈希函数得到 `k` 个位置：

1. 如果这 `k` 个位置上的状态为**0**那么该元素一定不在集合中。
2. 如果这 `k` 个位置上的状态都为**1**那么该元素可能在集合中。
