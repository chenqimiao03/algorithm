...
滑动过程：滑动窗口可以用**简单变量**或者**结构**来维持信息

求解大流程：求子数组在**每个位置开头**或者**结尾**情况下的答案

注意：

滑动窗口维持最大值或者最小值的**更新结构**
...
常用的缓存替换算法有：先进先出（First-In-First-Out，简称 FIFO）、最近最少使用（Least-Recently Used，简称 LRU）、最不经常用（Least-Frequently Used，简称 LFU）和随机替换。

### 最近最少使用缓存（LRUCache）

LRU 算法的基本思想是：总是选择近期最少使用的主存块被替换掉，这种替换算法能比较正确地反映程序的访问局部性。

### 最不经常使用缓存（LFUCache）

LFU 算法的基本思想是：替换掉 cache 中引用次数最少的块。
